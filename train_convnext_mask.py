...
class ConvNeXtBase4ch(torch.nn.Module):
	def __init__(self, num_classes: int):
		super().__init__()
		self.base = convnext_base(weights=ConvNeXt_Base_Weights.IMAGENET1K_V1)

		# Adapt first conv (stem conv) to 4 channels
		# In torchvision ConvNeXt, the stem is features[0][0], which is a Conv2d
		...
		self.base.classifier[2] = torch.nn.Linear(in_features, num_classes)
...
class LitConvNeXt(pl.LightningModule):
	def __init__(self, num_classes, lr: float = 1e-4, warmup_epochs: int = 0, hard_example_k: int = 16):
		super().__init__()
		self.model = ConvNeXtBase4ch(num_classes)
		self.lr = lr
	...
	def training_step(self, batch, batch_idx):
		...
		for si, l in zip(sample_indices, per_sample_loss.detach().cpu().tolist()):
			self.training_hard_examples.append(
				{"sample_index": si, "loss": float(l), "epoch": int(self.current_epoch)}
			)
		return loss
	...
	def on_train_epoch_end(self):
		...
		os.makedirs("hard_examples_convnext", exist_ok=True)
		df.to_csv(
			os.path.join("hard_examples_convnext", f"hard_examples_epoch_{self.current_epoch}.csv"),
			index=False,
		)
		self.training_hard_examples = []
...
def run_cv(
	...
	for fold, (train_idx, val_idx) in enumerate(skf.split(df, df["label"])):
		...
		logger = TensorBoardLogger("tb_logs", name=f"convnext_fold{fold+1}")
...
def run_single_split(
	...
	logger = TensorBoardLogger("tb_logs", name="convnext_single_split")
...
def parse_args():
	parser = argparse.ArgumentParser(
		description="ConvNeXt-Base training with masks as 4th channel"
	)

	parser.add_argument("--train-csv", type=str, default="data/train_labels.csv")
	parser.add_argument("--img-dir", type=str, default="data/train_data")
	parser.add_argument("--mask-dir", type=str, default="data/train_data")
	...
	parser.add_argument("--hard-example-k", type=int, default=16)
